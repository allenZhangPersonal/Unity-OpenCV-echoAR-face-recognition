...
public class RemoteTransformations : MonoBehaviour
{
    ...
    void getData(){
        ...
        if (entry.getAdditionalData().TryGetValue("z", out value))
        {
            ...
            camDetector camDetect = GameObject.Find("Camera").GetComponent<camDetector>();
            if(!camDetect.showModel){
                positionOffest.z = 160f; // Sets it behind canvas, acts like visibility
            }
        }
